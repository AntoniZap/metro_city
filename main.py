...
def main():
    ...
    while len(settlements) != 0:
        ...
        for i in indexes[:]:
            row_index = i[0]
            if (str(settlements.iloc[row_index, 2])+ " " +str(settlements.iloc[row_index, 4]) + " " + str(settlements.iloc[row_index, 5]) ) in settlements_to_metros:
                indexes.remove(i)
            else:
                col_index = i[1]
                settlements_to_metros[(str(settlements.iloc[row_index, 2])+ " " +str(settlements.iloc[row_index, 4]) + " " + str(settlements.iloc[row_index, 5]))] = metro.iloc[col_index, 2]
    ...
    print("Current hashmap len: " + str(len(settlements_to_metros)))
